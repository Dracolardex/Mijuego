...
func _animacion_ataque():
	...
	print("Ejecutando animación de ataque sobre: ", seleccion_objetivo.name)
	$AnimationPlayer.play("attack1")
	
	var offset_x = 100
	var offset_y = 0
	self.global_position = seleccion_objetivo.global_position + Vector2(offset_x, offset_y)

func _on_animation_player_animation_finished(anim_name):
	match anim_name:
		"attack1":
			print("Animación attack1 terminada")
			
			var enemigos = get_tree().get_nodes_in_group(enemigo)
			
			if enemigos.size() > 0:
				print("Aplicando daño de ", damage, " a: ", enemigos[0].name)
		...
		1:
			...
			if slot:
				if slot.get_child_count() > 0:
					...
					enemigo = "enemi001"
					print("Objetivo encontrado: ", seleccion_objetivo.name)
			else:
